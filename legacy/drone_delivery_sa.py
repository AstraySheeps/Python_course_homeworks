...
from common import (
    ...
    PENALTY_WEIGHT, COST_WEIGHT_DISTANCE, COST_WEIGHT_TIME,
)
...
def simulated_annealing(clients, dist_matrix, num_drones,
    ...
    # 时间窗模式下搜索空间更复杂，自动增加重启次数和每温迭代
    n_restarts = SA_MAX_RESTARTS + 2 if time_windows is not None else SA_MAX_RESTARTS
    n_iter_per_t = iter_per_t + 30 if time_windows is not None else iter_per_t
...
def plot_annealing_curve(all_cost_history, save_to_file=False,
    ...
    并在双Y轴上叠加温度下降曲线。
    """
    n_restarts = len(all_cost_history)
    restart_colors = ["#58a6ff", "#3fb950", "#f78166", "#d2a8ff", "#ffa657",
                      "#79c0ff", "#56d364", "#ff7b72", "#bc8cff", "#ffb347"]
    fig, axes = plt.subplots(n_restarts, 1, figsize=(14, 3.2 * n_restarts),
    ...
    for idx, (cost_hist, temp_hist) in enumerate(all_cost_history):
        ...
        ax.set_title(f"重启 {idx + 1}  |  最终最优代价: {final_cost:.2f}  |  "
...
def plot_convergence_summary(all_cost_history, save_to_file=False,
    ...
    ax.set_ylabel("最优代价", color=TEXT_SEC, fontsize=9)
...
def run_sa(num_drones=5, clients=None, output_dir=None, time_windows=None):
    """
