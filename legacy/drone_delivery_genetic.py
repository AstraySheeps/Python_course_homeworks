    ...
    while i < n:
        ...
        while i < n:
            ...
            if new_load <= max_capacity and new_dist <= max_distance:
                ...
                # 时间窗感知归组：若等待过长（>15时间单位），客户留到后续趟次
                if has_tw and route and (ready - arrival) > 10.0:
                    break  # 结束当前趟次，客户在下一轮外循环重新处理
        ...
        if route:
            if has_tw:
                # 趟内按 due_time 排序，使时间窗兼容的客户优先访问（EDD 规则）
                route.sort(key=lambda c: due_times[c])
                # 按排序后顺序重算距离、时间、惩罚
                sorted_dist = 0.0
                sorted_pos = depot_idx
                sorted_time = depart_time
                for c in route:
                    d = dist_matrix[sorted_pos, c + 1]
                    sorted_dist += d
                    arr = sorted_time + d / drone_speed
                    if arr > due_times[c]:
                        penalty += penalty_weight * (arr - due_times[c])
                    sorted_time = max(arr, ready_times[c]) + service_time
                    sorted_pos = c + 1
                current_dist = sorted_dist
                current_pos = sorted_pos
                current_time = sorted_time
...
def evaluate(individual, clients, dist_matrix, num_drones, time_windows=None):
    """适应度 = w1×总飞行距离 + w2×makespan + 约束惩罚（最小化）"""
    ...
    cost = COST_WEIGHT_DISTANCE * total_dist + COST_WEIGHT_TIME * makespan + penalty
    return (cost,)
...
def run_genetic_algorithm(clients, dist_matrix, num_drones, time_windows=None):
    ...
    # 时间窗模式下搜索空间更复杂，自动增加进化代数
    ngen = GA_NGEN + 100 if time_windows is not None else GA_NGEN
    ...
    print(f"\n遗传算法开始：无人机={num_drones} 架，种群={GA_POP_SIZE}，"
          f"迭代={ngen}，交叉率={GA_CXPB}，变异率={GA_MUTPB}")
    print("-" * 60)
...
def print_results(clients, routes, num_drones, save_to_file=False,
    ...
    lines.append(f"总完成时间(makespan): {makespan:.2f} 时间单位")
...
def _draw_stats_panel(ax, clients, routes, num_drones, drone_colors):
    ...
    summaries = [
        ...
        ("总里程", f"{total_dist:.1f}"), ("完成时间", f"{makespan:.1f}"),
    ]
    y = 0.88
...
def run_genetic(num_drones=5, clients=None, output_dir=None, time_windows=None):
    """
...
def main():
    ...
    parser.add_argument('--num-drones', type=int, default=5, help='无人机数量')
