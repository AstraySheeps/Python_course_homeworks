...
from common import (
    ...
    COST_WEIGHT_DISTANCE, COST_WEIGHT_TIME,
)


def _weighted_cost(result):
    """计算结果的加权代价 = w1×总距离 + w2×makespan"""
    return (COST_WEIGHT_DISTANCE * result['total_distance']
            + COST_WEIGHT_TIME * result.get('makespan', 0))


def print_comparison(greedy_result, genetic_result):
    """打印贪心 vs 遗传的指标对比，加权代价为主指标"""
    gc = _weighted_cost(greedy_result)
    ggc = _weighted_cost(genetic_result)

    print("\n" + "=" * 70)
    print("算法对比结果（目标: min w1×距离 + w2×makespan）")
    print("=" * 70)
    print(f"{'指标':<20} {'贪心算法':>20} {'遗传算法':>20}")
    print("-" * 70)
    print(f"{'加权代价 ★':<20} {gc:>20.2f} {ggc:>20.2f}")
    print(f"{'  总飞行距离':<20} {greedy_result['total_distance']:>20.2f} {genetic_result['total_distance']:>20.2f}")
    print(f"{'  makespan':<20} {greedy_result.get('makespan', 0):>20.2f} {genetic_result.get('makespan', 0):>20.2f}")
    print(f"{'总趟次':<20} {greedy_result['total_trips']:>20d} {genetic_result['total_trips']:>20d}")
    print(f"{'总配送次数':<20} {greedy_result['total_deliveries']:>20d} {genetic_result['total_deliveries']:>20d}")

    improvement = gc - ggc
    improvement_pct = improvement / gc * 100
    print("-" * 70)
    print(f"遗传算法相比贪心算法，加权代价减少: {improvement:.2f} ({improvement_pct:.1f}%)")
    print("=" * 70)
...
def plot_comparison(greedy_result, genetic_result, save_to_file=False, output_dir=None):
    ...
    for alg_name, result, ax in [("贪心算法", greedy_result, axes[0]),
        ...
        ax.set_title(f"{alg_name}\n距离: {total_dist:.1f}  |  makespan: {makespan:.1f}  |  趟次: {n_trips}",
...
def print_all_comparison(greedy_result, genetic_result, sa_result):
    ...
    print("\n" + "=" * 80)
    print("三种算法对比结果（目标: min w1×距离 + w2×makespan）")
    print("=" * 80)
    header = f"{'指标':<20}"
    ...
    print(f"{'加权代价 ★':<20}", end="")
    for _, r in results:
        print(f" {_weighted_cost(r):>18.2f}", end="")
    print()
    ...
    print(f"{'  makespan':<20}", end="")
    for _, r in results:
        print(f" {r.get('makespan', 0):>18.2f}", end="")
    print()
    ...
    baseline = _weighted_cost(greedy_result)
    for name, r in results:
        imp = baseline - _weighted_cost(r)
        imp_pct = imp / baseline * 100
        print(f"{name} 相比贪心基线，加权代价减少: {imp:+.2f} ({imp_pct:+.1f}%)")
    print("=" * 80)
...
def print_multi_seed_summary(all_results, algo_names, seeds, output_dir):
    ...
    print(f"{'加权代价 ★':<20}", end="")
    for key in active_algos:
        costs = [_weighted_cost(r) for r in all_results[key]]
        mean_c = np.mean(costs)
        std_c = np.std(costs, ddof=1) if len(costs) > 1 else 0
        print(f" {mean_c:>15.2f}±{std_c:>5.2f}", end="")
    print()
    ...
    print(f"{'  makespan':<20}", end="")
    for key in active_algos:
        mks = [r.get('makespan', 0) for r in all_results[key]]
        mean_m = np.mean(mks)
        std_m = np.std(mks, ddof=1) if len(mks) > 1 else 0
        print(f" {mean_m:>15.2f}±{std_m:>5.2f}", end="")
    print()
    ...
    if all_results.get('greedy'):
        greedy_costs = [_weighted_cost(r) for r in all_results['greedy']]
        greedy_mean = np.mean(greedy_costs)
        for key in ['genetic', 'sa']:
            if all_results.get(key):
                algo_costs = [_weighted_cost(r) for r in all_results[key]]
                algo_mean = np.mean(algo_costs)
                imp = greedy_mean - algo_mean
                imp_pct = imp / greedy_mean * 100
                print(f"{algo_names[key]} 相比贪心基线（均值），加权代价减少: "
                      f"{imp:+.2f} ({imp_pct:+.1f}%)")
    ...
    for key in active_algos:
        costs = [_weighted_cost(r) for r in all_results[key]]
        detail = ", ".join(f"{c:.2f}" for c in costs)
    ...
    if output_dir:
        ...
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write("多随机种子评估汇总（目标: min w1×距离 + w2×makespan）\n")
            f.write(f"权重: w1={COST_WEIGHT_DISTANCE}, w2={COST_WEIGHT_TIME}\n")
            f.write(f"种子: {seeds}\n")
            f.write(f"样本数: {len(seeds)}\n\n")
            for key in active_algos:
                costs = [_weighted_cost(r) for r in all_results[key]]
                dists = [r['total_distance'] for r in all_results[key]]
                makespans = [r.get('makespan', 0) for r in all_results[key]]
                trips = [r['total_trips'] for r in all_results[key]]
                f.write(f"{algo_names[key]}:\n")
                f.write(f"  加权代价: {np.mean(costs):.2f} ± {np.std(costs, ddof=1):.2f}\n")
                f.write(f"  总距离:   {np.mean(dists):.2f} ± {np.std(dists, ddof=1):.2f}\n")
                f.write(f"  makespan: {np.mean(makespans):.2f} ± {np.std(makespans, ddof=1):.2f}\n")
                f.write(f"  各次代价: {[f'{c:.2f}' for c in costs]}\n")
                f.write(f"  趟次:     {np.mean(trips):.1f} ± {np.std(trips, ddof=1):.1f}\n\n")
            if all_results.get('greedy'):
                greedy_mean = np.mean([_weighted_cost(r) for r in all_results['greedy']])
                for key in ['genetic', 'sa']:
                    if all_results.get(key):
                        algo_mean = np.mean([_weighted_cost(r) for r in all_results[key]])
                        imp_pct = (greedy_mean - algo_mean) / greedy_mean * 100
                        f.write(f"{algo_names[key]} 相比贪心加权代价提升: {imp_pct:.1f}%\n")
        print(f"\n汇总已保存: {filepath}")
...
def run_multi_seed_evaluation(args, output_dir):
    ...
    from drone_delivery import run_greedy, greedy_assignment, _assign_routes_to_drones
    ...
    for idx, seed in enumerate(seeds):
        ...
        if args.algo in ('greedy', 'both', 'all'):
            ...
            routes = _assign_routes_to_drones(routes, args.num_drones)
            ...
            all_results['greedy'].append({
                ...
                'total_distance': total_dist, 'makespan': makespan,
        ...
        if args.algo in ('genetic', 'both', 'all'):
            ...
            makespan = max(r['arrive_time'] for r in best_routes) if best_routes else 0
        ...
        if args.algo in ('sa', 'all'):
            ...
            all_results['sa'].append({
                'clients': clients, 'routes': best_routes, 'dist_matrix': dist_matrix,
                'total_distance': best_cost, 'makespan': makespan,
                'total_trips': len(best_routes),
                'total_deliveries': sum(r['deliveries'] for r in best_routes),
            })
        ...
        if is_last:
            ...
            if args.algo in ('greedy', 'both', 'all'):
                run_greedy(clients=clients, output_dir=output_dir,
                           time_windows=time_windows, num_drones=args.num_drones)
            if args.algo in ('genetic', 'both', 'all'):
