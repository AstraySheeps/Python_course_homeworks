...
from common import (
    ...
    PENALTY_WEIGHT, SERVICE_TIME, COST_WEIGHT_DISTANCE, COST_WEIGHT_TIME,
)
from drone_delivery import greedy_assignment, _assign_routes_to_drones
...
def plot_single_route(clients, routes, title="路径规划结果"):
    ...
    cost = COST_WEIGHT_DISTANCE * total_dist + COST_WEIGHT_TIME * makespan
    ax.set_title(f"{title}\n代价: {cost:.1f}  |  距离: {total_dist:.1f}  |  ms: {makespan:.1f}  |  趟次: {len(routes)}",
...
def plot_comparison_side_by_side(triples, names):
    ...
    for ax, (clients, routes), name in zip(axes, triples, names):
        ...
        makespan = max(r['arrive_time'] for r in routes) if routes else 0
        cost = COST_WEIGHT_DISTANCE * total_dist + COST_WEIGHT_TIME * makespan
        ax.set_title(f"{name}\n代价: {cost:.1f}  |  距离: {total_dist:.1f}  |  ms: {makespan:.1f}",
...
def plot_evolution_curve(logbook):
    ...
    ax.set_ylabel("适应度（加权代价）", color=TEXT_SEC, fontsize=9)
...
def plot_annealing_curves(all_cost_history):
    ...
    for idx, (cost_hist, temp_hist) in enumerate(all_cost_history):
        ...
        ax.set_ylabel("最优代价", color=TEXT_SEC, fontsize=7.5)
...
def plot_distance_bar_chart(results):
    ...
    for key, name in [('greedy', '贪心算法'), ('genetic', '遗传算法'), ('sa', '模拟退火')]:
        if key in results:
            r = results[key]
            cost = COST_WEIGHT_DISTANCE * r['total_distance'] + COST_WEIGHT_TIME * r.get('makespan', 0)
            names.append(name)
            values.append(cost)
    ...
    ax.set_ylabel("总飞行距离 / 加权代价", color=TEXT_SEC, fontsize=9)
    ax.set_title("算法代价对比（距离 + makespan加权）", color=TEXT_PRI, fontsize=11, fontweight='bold')
    plt.tight_layout()
    return fig
...
def main():
    ...
    with st.sidebar:
        ...
        n_clients = st.slider("客户点数量", 10, 50, 20, 5,
                              help="仿真生成的客户点总数")
        n_drones = st.slider("无人机数量", 3, 20, 5,
                             help="机队规模（遗传/退火算法使用）")
    ...
    if run_clicked:
        with st.spinner("🔄 正在生成数据并运行算法…"):
            ...
            if algo in ("全部对比", "贪心算法"):
                ...
                routes = _assign_routes_to_drones(routes, n_drones)
                results['greedy'] = {
                    'clients': clients,
                    'routes': routes,
    ...
    if st.session_state.results is not None:
        ...
        for col, (key, name) in zip(cols, existing):
            ...
            cost = COST_WEIGHT_DISTANCE * r['total_distance'] + COST_WEIGHT_TIME * r.get('makespan', 0)
            with col:
                st.metric(label=name, value=f"{cost:.1f}",
                          delta=f"距离:{r['total_distance']:.1f}  |  makespan:{r.get('makespan', 0):.1f}")
        ...
        if algo_label == "全部对比":
            ...
            # 对比表
            st.subheader("📋 指标对比")
            greedy_cost = (COST_WEIGHT_DISTANCE * results.get('greedy', {}).get('total_distance', 0)
                          + COST_WEIGHT_TIME * results.get('greedy', {}).get('makespan', 0))
            import pandas as pd
            ...
            for key, name in [('greedy', '贪心算法'), ('genetic', '遗传算法'), ('sa', '模拟退火')]:
                if key in results:
                    r = results[key]
                    cost = COST_WEIGHT_DISTANCE * r['total_distance'] + COST_WEIGHT_TIME * r.get('makespan', 0)
                    imp = (greedy_cost - cost) / greedy_cost * 100 if greedy_cost > 0 else 0
                    table_data.append({
                        '算法': name,
                        '加权代价 ★': f"{cost:.2f}",
                        '总距离': f"{r['total_distance']:.2f}",
                        'makespan': f"{r.get('makespan', 0):.2f}",
                        '趟次': r['total_trips'],
                        f'vs贪心提升': f"{imp:+.1f}%",
                    })
